...
function(x,obs=1, ...){
  ...
  quant <- predict.fast(x,quantiles=quantiles,obs=obs)

  or <- order(quant[,2])
  n <- length(x$origObs)
  plot(quant[or,2],x$origObs[or],pch=20,xlab="predicted median values", ylab ="observed response",type="n",main="90 % prediction intervals on out-of-bag data") 
